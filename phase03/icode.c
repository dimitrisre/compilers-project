...
//extern struct expr * new_call_var;
...
static Quad *quad_array = (Quad *) 0;
static unsigned int total = 0;
static int Currquad  = 0;
...
char *op_names[]= {
			...
			"funcend"      , "tablecreate"  , "nop"			,
			"tablegetelem" , "tablesetelem" , "jump" ,
	        "jump_while", "jump_break", "jump_cont","jump_for"
...
void print_quads(){
	...
	for(i=0; i<Currquad; i++){
	   ...
	   if( quad_array[i].op == call ){
		...
		if(quad_array[i].op == jump || quad_array[i].op == jump_while|| quad_array[i].op == jump_for||quad_array[i].op == jump_break
		   ||quad_array[i].op == jump_cont ){
