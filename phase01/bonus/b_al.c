...
int main(int argc,char **argv){
	...
	while( ( i=alpha_yylex( token_ptr )) !=ENDOF ){
		...
		switch(i){
			...
			case INTCONST:
				fprintf(output,"lineno:%d , ayxwn:%d ,  INTCONST (%d): \"%d\"\n",new->lineno,count,new->type,new->int_value);
				break;
			case FLOATCONST:
				fprintf(output,"lineno:%d , ayxwn:%d , FLOATCONST(%d): \"%lf\"\n",new->lineno,count,new->type,new->d_value);
				break;
			case IF_KEYWORD:
			case FOR_KEYWORD:
			case ELSE_KEYWORD:
			...
			fprintf(output,"lineno:%d , ayxwn:%d , KEYWORD(%d): \"%s\"\n",new->lineno,count,new->type,new->s_value);
				break;
			...
			case LESS_OP:
				...
				fprintf(output,"lineno:%d , ayxwn:%d , OPERATOR(%d): \"%s\"\n",new->lineno,count,new->type,new->s_value);
				break;
			case STRING:
				fprintf(output,"lineno:%d , ayxwn:%d , STRING(%d): \"%s\"\n",(new->lineno),count,new->type,new->s_value);
				break;
			case L_PAR:
			case R_PAR: 
			case L_AG:
			...
			case D_FULLST: 
				...
				fprintf(output,"lineno:%d , ayxwn:%d , PANCTUATION(%d): \"%s\"\n",new->lineno,count,new->type,new->s_value);
				break;
			case ID:
				fprintf(output,"lineno:%d , ayxwn:%d , IDENTIFIER(%d): \"%s\"\n",new->lineno,count,new->type,new->s_value);
				break;
			case ILLEGAL:
				fprintf(output,"lineno:%d , ayxwn:%d , ILLEGAL(%d): \"%s\"\n",new->lineno,count,new->type,new->s_value);
				break;
			default:	
				break;
		}
